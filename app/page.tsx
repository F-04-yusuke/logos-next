...
export default function Home() {
  ...
  // 人気トピック（取得済みデータの中から posts_count 上位5件）
  ...
  const fetchTopics = useCallback((page: number, sortVal: SortOption) => {
    setLoading(true);
    setError(false);
    fetch(`${BASE}/api/topics?sort=${sortVal}&page=${page}`)
      .then((r) => { if (!r.ok) throw new Error(); return r.json(); })
      .then((data: TopicsResponse) => {
        setTopics(data.data ?? []);
        setCurrentPage(data.current_page);
        setLastPage(data.last_page);
        // 人気トピック: 全データから posts_count 降順 top5
  ...
  useEffect(() => {
    fetch(`${BASE}/api/categories`)
      .then((r) => r.json())
      .then((data: Category[]) => {
        // parent_id === null のみタブとして表示
        const roots = data.filter((c) => c.parent_id === null);
        setCategories(roots);
        if (roots.length > 0) setActiveTab(roots[0].id);
      })
      .catch(() => {});
  }, []);

  // ── タブ用トピックフェッチ（カテゴリ別・最新5件） ──
  useEffect(() => {
    if (categories.length === 0) return;
    categories.forEach((cat) => {
      fetch(`${BASE}/api/topics?category=${cat.id}&per_page=5`)
        .then((r) => r.json())
        .then((data: TopicsResponse) => {
