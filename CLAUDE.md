...
### Blade 参照ルール（Phase 5 時点・全18ページ移植完了済み）

| 作業種別 | Blade 参照 | 理由 |
|---|---|---|
| **新しいビジネス機能の追加**（投稿制限・権限・フロー等） | **必要** | Blade版に仕様が定義されているため |
| **未実装ページの新規移植** | **必要** | 機能漏れ防止のため |
| **技術的リファクタリング**（SSR化・型改善・コンポーネント分離・パフォーマンス最適化） | **不要** | 既実装コードを技術的に組み替えるだけ。APIの挙動はコードで確認済み |
| **UIデザイン変更**（色・ホバー・スペーシング等） | **不要** | Next.js版がBladeを超えており参照価値なし |

- Blade に存在するビジネスロジック（返信制限・補足ルール・権限等）を勝手に削除・省略・簡略化しない
- ビルド成功だけで完了としない。新機能追加時は Blade との機能差分を必ず確認する
- 実装前に以下も必読（新機能追加時）:
...
| `.claude/skills/progress-phase4.md` | Phase 4 完了記録（UI/UX改善・Session 20〜） |
