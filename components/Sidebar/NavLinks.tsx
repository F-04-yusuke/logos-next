...
export default function NavLinks({
  ...
  const iconClass = (href: string, exact = true) => {
    const active = exact ? pathname === href : pathname.startsWith(href);
    return `w-5 h-5 transition-colors ${active ? "text-white" : "text-gray-400 group-hover:text-white"}`;
  };

  return (
    <div
      className={[
        "py-4 px-3 space-y-3 w-64",
      ...
      <ul className="space-y-1">
        ...
        <li>
          <Link href="/category-list" className={navClass("/category-list")}>
            <svg className={iconClass("/category-list")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9.776c.112-.017.227-.026.344-.026h15.812c.117 0 .232.009.344.026m-16.5 0a2.25 2.25 0 00-1.883 2.542l.857 6a2.25 2.25 0 002.227 1.932H19.05a2.25 2.25 0 002.227-1.932l.857-6a2.25 2.25 0 00-1.883-2.542m-16.5 0V6A2.25 2.25 0 016 3.75h3.879a1.5 1.5 0 011.06.44l2.122 2.12a1.5 1.5 0 001.06.44H18A2.25 2.25 0 0120.25 9v.776" />
            </svg>
            <span className="ml-3">カテゴリ一覧</span>
          </Link>
        </li>
      </ul>
        ...
        <>
              ...
              {bookmarks.length === 0 ? (
                ...
                bookmarks.map((t) => (
                  <li key={t.id}>
                    ...
                    >
                      <span className="w-6 h-6 flex items-center justify-center text-xs font-bold text-gray-400 border border-gray-600 rounded shrink-0 group-hover:bg-gray-600 transition-colors">
                        {t.title.charAt(0)}
                      </span>
                      <span className="ml-2 text-sm truncate">{t.title}</span>
                    </Link>
                  </li>
                ))
              )}
            ...
            <ul className="space-y-1 text-sm">
              <li>
                <Link href="/dashboard" className={navClass("/dashboard")}>
                  <svg className={iconClass("/dashboard")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <li>
                <Link href="/notifications" className={navClass("/notifications")}>
                  <div className="relative">
                    <svg className={iconClass("/notifications")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <li>
                <Link href="/likes" className={navClass("/likes")}>
                  <svg className={iconClass("/likes")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <li>
                <Link href="/history" className={navClass("/history")}>
                  <svg className={iconClass("/history")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <li>
                {isPro ? (
                  <Link href="/topics/create" className={navClass("/topics/create")}>
                    <svg className={iconClass("/topics/create")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <button
                    ...
                    className="w-full flex items-center p-2 rounded-lg hover:bg-gray-800 text-gray-500 transition-colors group cursor-pointer"
                  >
          ...
          <div>
            <h3 className="px-2 text-sm font-semibold text-yellow-500 uppercase tracking-wider mb-2 flex items-center">
              分析ツール
              <span className="ml-1.5 text-[9px] bg-yellow-500 text-white px-1 py-0.5 rounded font-bold tracking-wider">PRO</span>
            </h3>
          ...
          <ul className="space-y-1 text-sm">
            <li>
              <Link href="/profile" className={navClass("/profile")}>
                <svg className={iconClass("/profile")} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
